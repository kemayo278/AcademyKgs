      ...
      <div className="container flex h-16 items-center space-x-4 sm:justify-between sm:space-x-0">
        <div className="flex gap-6 md:gap-10">
          <Link href="/" className="flex items-center space-x-2">
            <div className="relative h-15 w-31 overflow-hidden">
              <Image
                src={theme === "dark" ? "/logo-dark.png" : "/logo-light.png"}
                width={120}
                height={40}
                alt="Kokitech Academy Logo"
                className="absolute inset-0 object-cover mt-0"
              />
            </div>
            {/* <span className="inline-block font-bold">Kokitech Academy</span> */}
          </Link>
          <nav className="hidden md:flex gap-6 mt-5">
            {routes.map((route) => (
              <Link
                key={route.href}
                href={route.href}
